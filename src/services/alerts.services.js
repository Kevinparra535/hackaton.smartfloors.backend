...
class AlertService {
  ...
  detectAnomalies(currentData, history) {
    ...
    const occupancyAnomaly = this.checkOccupancy(currentData.occupancy, history);
    if (occupancyAnomaly) anomalies.push(occupancyAnomaly);

    // Detectar temperatura anormal
    const temperatureAnomaly = this.checkTemperature(currentData.temperature, currentData.occupancy);
    ...
    const powerAnomaly = this.checkPowerConsumption(currentData.powerConsumption, currentData.occupancy);
  ...
  checkOccupancy(occupancy, history) {
    ...
    if (occupancy >= this.thresholds.occupancy.veryHigh) {
      return {
        ...
        recommendation: 'Activar ventilación adicional y monitorear capacidad máxima. Considerar redistribuir personas a otros pisos.',
    ...
    if (occupancy >= this.thresholds.occupancy.high) {
      return {
        ...
        recommendation: 'Preparar sistemas de ventilación y monitorear confort térmico.',
        timestamp: new Date().toISOString(),
      };
    }
    ...
    if (history && history.length >= 7) {
      const recentAvg = this.calculateAverage(history.slice(-7).map((h) => h.occupancy));
      ...
      if (deviation > 30 && occupancy > recentAvg) {
        return {
          ...
          recommendation: 'Verificar si hay un evento programado. Ajustar climatización preventivamente.',
          timestamp: new Date().toISOString(),
        };
      }
    }
  ...
  checkTemperature(temperature, occupancy) {
    if (temperature >= this.thresholds.temperature.veryHigh) {
      return {
        ...
        recommendation: 'Activar aire acondicionado de inmediato. Reducir ocupación si es posible.',
        timestamp: new Date().toISOString(),
      };
    }
    ...
    if (occupancy > 70 && temperature < 21) {
      return {
        ...
        recommendation: 'Verificar sistema de climatización. Posible sobre-enfriamiento.',
        timestamp: new Date().toISOString(),
      };
    }
  ...
  checkHumidity(humidity) {
    if (humidity >= this.thresholds.humidity.veryHigh) {
      return {
        ...
        recommendation: 'Activar deshumidificadores. Alta humedad puede afectar confort y equipos.',
        timestamp: new Date().toISOString(),
      };
    }
  ...
  checkPowerConsumption(powerConsumption, occupancy) {
    if (powerConsumption >= this.thresholds.powerConsumption.veryHigh) {
      return {
        ...
        recommendation: 'Revisar sistemas eléctricos. Posible mal funcionamiento o desperdicio energético.',
        timestamp: new Date().toISOString(),
      };
    }
    ...
    if (powerConsumption >= this.thresholds.powerConsumption.high) {
      return {
        ...
        recommendation: 'Optimizar uso de equipos. Apagar luces y dispositivos innecesarios.',
        timestamp: new Date().toISOString(),
      };
    }
    ...
    if (occupancy < 20 && powerConsumption > 100) {
      return {
        ...
        recommendation: 'Verificar equipos encendidos innecesariamente. Posible ahorro energético.',
        timestamp: new Date().toISOString(),
      };
    }
  ...
  checkSuddenChanges(currentData, history) {
    ...
    const occupancyChange = Math.abs(currentData.occupancy - previous.occupancy);
    if (occupancyChange > 30) {
      return {
        ...
        recommendation: 'Monitorear situación. Verificar si corresponde a evento programado.',
        timestamp: new Date().toISOString(),
      };
    }
    ...
    if (tempChange > 3) {
      return {
        ...
        recommendation: 'Verificar sistema de climatización. Cambio inusualmente rápido.',
        timestamp: new Date().toISOString(),
      };
    }
